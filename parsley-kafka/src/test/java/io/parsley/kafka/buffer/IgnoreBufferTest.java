...
class IgnoreBufferTest {
    ...
    void drainReleasesOnlySatisfiedRecords() {
        IgnoreBuffer<String, String> buffer = new IgnoreBuffer<>(SERIALISER);
        buffer.add(record("early", new KafkaVectorClock(Map.of(P0, 2L))));
        buffer.add(record("late", new KafkaVectorClock(Map.of(P0, 10L))));

        List<ConsumerRecord<String, String>> released = buffer.drain(new KafkaVectorClock(Map.of(P0, 2L)));
    ...
    void drainNoHeaderTreatedAsEmptyClock() {
        ...
        List<ConsumerRecord<String, String>> released = buffer.drain(KafkaVectorClock.empty());
        assertEquals(1, released.size());
    }
