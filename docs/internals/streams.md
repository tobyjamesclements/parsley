...
3. Add `parsley-causal-dependencies` from `frontier.get().toBytes()` (the `Supplier<ParsleyVectorClock>` is read at call time, and supplies the completeness frontier).
