...
Each forwarded record is stamped live at forward time with the outbound stamp (`ParsleyChannels.stamp()`: completeness ∪ ownOutputs ∪ highestDelivered) — the max-merge of this node's own contiguous frontier, every input channel's advertised clock, and its own acked output positions, carrying transitive ancestry downstream (see [Causal consistency model](../foundations/causal-consistency.md)). The delivery gate itself is the node's own contiguous frontier: a record is delivered only once this node has itself delivered every coordinate it depends on; an advertised claim feeds only the stamp. The same stamp value is attached to forwarded records and null messages.
