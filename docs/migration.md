...
Turn on `CausalConsumers`/`CausalProcessors` against the full topic set immediately. Records from
producers that haven't adopted `CausalProducer` yet pass straight through, unbuffered, stamped
`SATISFIED`. Records from producers that have already adopted it are held until their dependencies
are satisfied and benefit from the guarantee right away — the two kinds of producer coexist on the
same topics without any configuration distinguishing them.

### Phase 2 — migrate producers one service at a time

Replace legacy producers with `CausalProducer` one service at a time. As each service migrates, its
records start arriving with a valid header and are held until their dependencies are satisfied. The
rest of the cluster benefits from the guarantee immediately; remaining legacy services continue to
...
- The frontier always advances on delivery regardless of whether a record carried a header, so
  records buffered downstream of a still-unmigrated producer are not permanently stalled.
