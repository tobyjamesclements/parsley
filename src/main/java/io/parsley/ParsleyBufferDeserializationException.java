 ...
 *
 * <p>How a decode failure is handled follows the application's configured deserialization exception
 * handler (read once at processor {@code init()}): the default {@code LogAndFail} fails fast — the
 * offending entry stays in the buffer store (the forward path deserialises before it removes), so the
 * record is preserved for recovery once the registry is fixed or rolled back — while {@code
 * LogAndContinue} skips it (dropped, logged, counted as a violation). Either way this is a {@link
 * RuntimeException}, fatal only to the owning Streams task, never crashing the JVM; and the
