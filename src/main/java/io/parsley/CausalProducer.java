 ...
 * the {@code KafkaProducer} it wraps. {@link #send} builds a fresh record carrying the dependencies
 * header and never mutates the {@link org.apache.kafka.clients.producer.ProducerRecord} passed in,
 * so the caller's record may be reused.
 *
...
public interface CausalProducer<K, V> extends AutoCloseable {
     ...
     *
     * @param record       the record to send; must not be {@code null}
     * @param dependencies the causal dependencies to embed; must not be {@code null}
     * @return a {@link Future} for the {@link RecordMetadata} of the sent record
     */
    Future<RecordMetadata> send(ProducerRecord<K, V> record, CausalDependencies dependencies);

    /**
     * Sends a record with {@code dependencies} attached, invoking {@code callback} on completion.
     *
     * @param record       the record to send; must not be {@code null}
     * @param dependencies the causal dependencies to embed; must not be {@code null}
     * @param callback     the callback invoked on send completion or failure; may be {@code null}
     * @return a {@link Future} for the {@link RecordMetadata} of the sent record
     */
    Future<RecordMetadata> send(ProducerRecord<K, V> record, CausalDependencies dependencies, Callback callback);
