...
import static io.parsley.streams.internal.CausalProcessor.CLOCK_HEADER;
import static io.parsley.streams.internal.CausalProcessor.FRONTIER_KEY;
import static io.parsley.streams.internal.CausalProcessor.FRONTIER_STORE;
...
class CausalProcessorRestartTest {
     ...
     * Two-phase test:
     *
     * <p><b>Phase 1</b> — process R1 and R2 (no deps) to advance the frontier to {@code {input/0:1}}.
     * Verify the frontier bytes are persisted in the state store.
     *
     * <p><b>Phase 2</b> — create a new processor whose state store is pre-seeded with the bytes
     * from phase 1 (mimicking RocksDB state surviving a restart). Verify:
     * <ul>
     *   <li>A — R3 with dep {@code {input/0:1}} is forwarded immediately (frontier restored).
     *   <li>B — R_held with dep {@code {input/0:3}} is buffered, then released in causal order
     *       once R_advance1 and R_advance2 push the frontier to {@code {input/0:3}}.
     * </ul>
     */
    @Test
    void frontierPersistedAndRestoredAcrossRestart() {
        ...
        // Pre-seed the store with the persisted bytes before init() runs — same as RocksDB restoration.
        ...
        proc2.init(ctx2);  // reads frontier {input/0:1} from store2
        ...
        proc2.process(record("R_advance1", noop()));  // frontier → {input/0:2}
        ctx2.setRecordMetadata(INPUT, 0, 3);
        proc2.process(record("R_advance2", noop()));  // frontier → {input/0:3}, drains R_held
    ...
    private CausalProcessor<String, String> processor() {
        return new CausalProcessor<>(
                ...
                (CausalViolationHandler) (rec, reason) -> violations.add(reason),
                SERIALISER,
                rec -> {});
    }
    ...
    private static VectorClock noop() { return Map::of; }

    private static VectorClock clock(Map<Partition, Long> positions) { return () -> positions; }
