...
class DeadLetterBufferTest {
    ...
    void drainReleasesRecordWithSatisfiedClock() {
        ...
        DeadLetterBuffer<String, String> buffer = new DeadLetterBuffer<>(SERIALISER, sink::add);
    ...
    void evictCallsHandlerWithLimitReachedForEachRecord() {
        DeadLetterBuffer<String, String> buffer = new DeadLetterBuffer<>(SERIALISER, r -> {});
        buffer.add(record("k1", new ImmutableVectorClock(Map.of(P0, 99L))));
        buffer.add(record("k2", new ImmutableVectorClock(Map.of(P0, 100L))));
    ...
    void evictReturnsEmptyList() {
        ...
        List<ConsumerRecord<String, String>> result = buffer.evict(null, (r, reason) -> {});
